...
export const useCourseContent = (courseId?: string) => {
    ...
    const createContent = useCallback(async (data: CourseContentEdit) => {
    ...
    const updateContent = useCallback(async (id: string, data: CourseContentEdit) => {
        setLoading(true)
        try {
    ...
    return { contents, loading, fetchContent, createContent, updateContent, deleteContent }
}
